...
interface Props {
  ...
  childrenPosition?: 'left' | 'right'; // Deprecated - Layout is now stacked
...
const AdvancedFilterBar: React.FC<Props> = ({
  ...
  return (
    <div className={`sticky top-0 z-20 bg-white dark:bg-[#1a202c] border-b border-slate-200 dark:border-slate-800 shadow-sm px-6 py-4 flex flex-col gap-4 ${className}`}>
        ...
        <div className="flex flex-col sm:flex-row sm:items-baseline gap-2 sm:gap-4">
            ...
            {subtitle && (
                <p className="text-[14px] text-[#86868B] dark:text-slate-400 font-medium whitespace-nowrap">
                    {subtitle}
                </p>
            )}
        </div>

        {/* ROW 2: Middle Content (Tabs or Search) */}
        {children && (
        ...
        {/* ROW 3: Tools (Filters & Actions) - Aligned Left */}
        <div className="flex flex-wrap items-center gap-3 w-full justify-start">
            ...
            {/* Actions Seperator */}
            {actions && <div className="h-6 w-px bg-slate-200 dark:bg-slate-700 hidden md:block shrink-0 mx-1"></div>}
