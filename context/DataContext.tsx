...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = (phone: string, pass: string) => { 
      if (phone === '0938806341' && pass === '123456') {
          ...
          setIsAuthenticated(true); setCurrentUser(user); localStorage.setItem('german_plus_auth', 'true'); localStorage.setItem('german_plus_user', JSON.stringify(user));
          return true;
      }
      return false;
  };
  const logout = () => { setIsAuthenticated(false); setCurrentUser(null); localStorage.removeItem('german_plus_auth'); localStorage.removeItem('german_plus_user'); };
  const handleRoleChange = (newRole: UserRole) => { if (currentUser) { setCurrentUser({ ...currentUser, role: newRole }); } };
  const startSimulation = (role: UserRole) => { if (currentUser) { if (!originalRole) setOriginalRole(currentUser.role); setCurrentUser({ ...currentUser, role }); } };
  const stopSimulation = () => { if (currentUser && originalRole) { setCurrentUser({ ...currentUser, role: originalRole }); setOriginalRole(null); } };
  const hasPermission = (permission: PermissionKey): boolean => { if (!currentUser) return false; const perms = ROLE_PERMISSIONS[currentUser.role]; return perms?.includes(permission) || false; };
  ...
  const calculateFinancials = (startDate?: Date, endDate?: Date): FinancialStats => {
      let filteredFinance = finance; let filteredTuition = tuition;
      if (startDate && endDate) {
          const start = new Date(startDate); start.setHours(0,0,0,0); const end = new Date(endDate); end.setHours(23,59,59,999);
          filteredFinance = finance.filter(f => { const d = new Date(f.date); return d >= start && d <= end; });
          filteredTuition = tuition.filter(t => { const d = new Date(t.dueDate); return d >= start && d <= end; });
      }
  ...
  return (
    <DataContext.Provider value={{
      ...
      isAuthenticated, currentUser, discrepancies, testResults, reconcileData, runSystemDiagnostics, calculateFinancials, recalculateSchedule, generateClassSessions, updateScheduleChain,
