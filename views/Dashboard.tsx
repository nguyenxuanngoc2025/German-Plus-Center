...
import Header from '../components/Header'; // Restore Header for mobile nav
...
const Dashboard: React.FC = () => {
  ...
  const { students, leads, finance, classes, currentUser, calculateFinancials, settings } = useData();
  ...
  const marketingStats = useMemo(() => {
      ...
      if (filters.source !== 'all') currentLeads = currentLeads.filter(l => l.source.includes(filters.source)); 
      ...
      return {
          current: curr,
          previous: prev,
          growth: { total: getGrowth(curr.total, prev.total), rate: Math.round(curr.rate - prev.rate) },
          sourceData,
          failData
      };
  ...
  const academicStats = useMemo(() => {
      ...
      if (filters.classType !== 'all') filteredClasses = classes.filter(c => c.mode === filters.classType);
      ...
      return {
          ...
          occupancyRate,
          filteredClasses,
          enrollment: { current: currEnroll, previous: prevEnroll, growth: getGrowth(currEnroll, prevEnroll) }
      };
  ...
  const GrowthBadge = ({ value, isCurrency = false, isInverse = false }: { value: number, isCurrency?: boolean, isInverse?: boolean }) => {
      ...
      if (isInverse) colorClass = isPositive ? 'text-red-600 bg-red-50' : 'text-emerald-600 bg-emerald-50';
      ...
      return (
          <div className={`flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-md ${colorClass} w-fit`}>
              <span className="material-symbols-outlined text-[12px]">{isNeutral ? 'remove' : isPositive ? 'trending_up' : 'trending_down'}</span>
  ...
  return (
    <div className="flex-1 flex flex-col h-full min-w-0 bg-[#f8fafc] dark:bg-background-dark relative font-display">
      {/* Hidden Mobile Header for Menu Access */}
      <div className="md:hidden">
          <Header title={settings.systemName} />
      </div>

      <AdvancedFilterBar 
        title="Tổng quan"
        subtitle="Báo cáo hiệu suất & Tăng trưởng"
        ...
        children={
            <div className="flex p-1 bg-slate-100 dark:bg-slate-800 rounded-lg shrink-0">
                {[
                    ...
                    { id: 'academic', label: 'Giáo vụ', icon: 'school' }
                ].map((tab) => (
                    <button
                        key={tab.id}
        ...
        actions={
            <>
                {activeTab === 'finance' && (
                    <button onClick={() => handleExport('finance')} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition-colors shadow-sm whitespace-nowrap h-9">
                ...
                {activeTab === 'marketing' && (
                    <>
                        <button onClick={() => setIsAddLeadOpen(true)} className="flex items-center gap-2 px-3 py-2 bg-secondary hover:bg-orange-600 text-white rounded-lg text-xs font-bold shadow-sm transition-colors whitespace-nowrap h-9">
                            <span className="material-symbols-outlined text-[18px]">add</span>
                            <span className="hidden sm:inline">Thêm Lead</span>
                        </button>
                        <button onClick={() => handleExport('marketing')} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition-colors shadow-sm whitespace-nowrap h-9">
                            <span className="material-symbols-outlined text-[18px]">download</span>
                        </button>
                    </>
                )}
                {activeTab === 'academic' && (
                    <button onClick={() => handleExport('academic')} className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition-colors shadow-sm whitespace-nowrap h-9">
                        <span className="material-symbols-outlined text-[18px]">download</span>
                        <span className="hidden sm:inline">Xuất BC</span>
                    </button>
                )}
      ...
      <div className="flex-1 overflow-y-auto p-6 scroll-smooth">
        ...
        <div className="max-w-[1600px] mx-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
                    ...
                    <div className="bg-white dark:bg-[#1a202c] p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-[380px]">
                        ...
                        <ResponsiveContainer width="100%" height="90%">
                            <AreaChart data={financeChartData}>
                                ...
                                {filters.isCompare && <Area type="monotone" dataKey="previous" name="Kỳ trước" stroke="#cbd5e1" strokeWidth={2} strokeDasharray="5 5" fill="none" />}
            ...
            {/* ... Marketing and Academic Tabs (unchanged logic) ... */}
            {activeTab === 'marketing' && (
                <div className="flex flex-col gap-5">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <StatCard label="Tổng Lead thu được" value={marketingStats.current.total} icon="campaign" color="blue" tooltip="Tổng số khách hàng tiềm năng..." subtext={<GrowthBadge value={marketingStats.growth.total} />} onClick={() => setDrillType('leads_total')} />
                        <StatCard label="Tỷ lệ chuyển đổi" value={`${marketingStats.current.rate.toFixed(1)}%`} icon="sync_alt" color="green" tooltip="Tỷ lệ Leads chuyển thành Học viên..." subtext={<GrowthBadge value={marketingStats.growth.rate} isCurrency={true} />} />
                        <StatCard label="Sẵn sàng (Ready)" value={marketingStats.current.ready} icon="verified" color="teal" tooltip="Số lượng Leads có khả năng chốt cao." subtext="Tiềm năng chốt cao" />
                    </div>
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="bg-white dark:bg-[#1a202c] p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-[380px]">
                            ...
                            {marketingStats.current.total > 0 ? (
                                <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={marketingStats.sourceData} cx="50%" cy="50%" innerRadius={70} outerRadius={100} paddingAngle={5} dataKey="value">{marketingStats.sourceData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} stroke="none" />)}</Pie><Tooltip /><Legend verticalAlign="bottom" height={36} /></PieChart></ResponsiveContainer>
                        ...
                        <div className="bg-white dark:bg-[#1a202c] p-5 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm h-[380px]">
                            ...
                            {marketingStats.failData.length > 0 ? (
                                <ResponsiveContainer width="100%" height="100%"><BarChart data={marketingStats.failData} layout="vertical" margin={{left: 20}}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" hide /><YAxis dataKey="name" type="category" width={120} tick={{fontSize: 11}} /><Tooltip /><Bar dataKey="value" fill="#EF4444" radius={[0, 4, 4, 0]} barSize={20} /></BarChart></ResponsiveContainer>
            ...
            {activeTab === 'academic' && (
                <div className="flex flex-col gap-5">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <StatCard label="Tổng Học viên Active" value={academicStats.activeStudents} icon="groups" color="blue" tooltip="Số lượng học viên đang học..." subtext="Học viên đang theo học" onClick={() => setDrillType('students_active')} />
                        <StatCard label="Tuyển sinh mới" value={academicStats.enrollment.current} icon="person_add" color="purple" tooltip="Số lượng học viên mới..." subtext={<GrowthBadge value={academicStats.enrollment.growth} />} />
                        <StatCard label="Tỷ lệ Lấp đầy" value={`${academicStats.occupancyRate}%`} icon="chair" color="green" tooltip="Tỷ lệ chỗ ngồi thực tế..." subtext={<div className="w-full bg-emerald-100 h-1.5 rounded-full mt-2 overflow-hidden max-w-[100px]"><div className="bg-emerald-500 h-full" style={{width: `${academicStats.occupancyRate}%`}}></div></div>} />
                    </div>
                    <div className="bg-white dark:bg-[#1a202c] border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
                        <div className="px-5 py-3 border-b border-slate-100 dark:border-slate-700"><h4 className="font-bold text-slate-800 dark:text-white">Hiệu suất Lớp học</h4></div>
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 uppercase font-bold text-xs">
                                <tr><th className="px-5 py-3">Tên lớp</th><th className="px-5 py-3">Giáo viên</th><th className="px-5 py-3 text-center">Sĩ số</th><th className="px-5 py-3 text-center">Tiến độ</th><th className="px-5 py-3 text-center">Trạng thái</th></tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                                {academicStats.filteredClasses.map(cls => (
                                    <tr key={cls.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                        ...
                                        <td className="px-5 py-3"><div className="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden"><div className="bg-primary h-full" style={{width: `${cls.progress}%`}}></div></div><p className="text-[10px] text-center mt-1 text-slate-500">{cls.progress}%</p></td>
                                        <td className="px-5 py-3 text-center"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${cls.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{cls.status}</span></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
